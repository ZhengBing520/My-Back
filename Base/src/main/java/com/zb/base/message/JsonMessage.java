...
public class JsonMessage<T> implements Serializable{
    ...
    public static <T> JsonMessage merge(JsonMessage<T> ...jsonMessages) {
        if(jsonMessages.length == 0){
            return JsonMessage.success();
        }
        StringBuilder message = new StringBuilder();
        for(JsonMessage jm : jsonMessages){
            if(!jm.result){
                message.append(jm.message);
                message.append("\n");
            }
        }
        if(StringUtil.hasLength(message)){
            return JsonMessage.fail(message.toString());
        }else {
            return JsonMessage.success();
        }
    }
